...
#                print(X[i,d])
            #    #X[i,d] = lb[0, d] + (ub[0, d] - lb[0, d]) * rand()
                ...
                X[i,d] = lb[0, d] + 0.4 + (ub[0, d] - thres) * rand()
                #print(X[i,d])
...
def DE(xtrain, ytrain, opts, cfg, run, results):
    ...
    mutation_rate_T   = [0.3] * N  ### Initial mutation rate for jDE
    ...
    while t < max_iter:  
        ...
        for i in range(N):    
            ...
            #if t <= max_iter*0.2:
            #if i == 1:
            #    print(mutation_rate_T[i], crossover_rate_T[i])
        ...
        for i in range(N):
            ...
            if cost[1] < cost[0]:
                # print(fit_features[i,0], fit_featureU)
                # print(pareto_parent[i], pareto[i])
                #print(fit[i,0], fitU)
                # print(X[i,:], U[i,:])
                X[i,:]   = U[i,:]
                fit[i,0] = fitU
            ...
            #     crossover_rate[i] = crossover_rate_T[i]
            if fit[i,0] > fitG:
                #print("Best anterior", fitG)
                #print("Best atual", fit[i,0])
                flag_best = 1
                Xgb[0,:] = X[i,:]
                fitG     = fit[i,0]   
                fitF1 = fit[i,0]
